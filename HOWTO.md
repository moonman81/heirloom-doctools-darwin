# HOWTO — install and use Heirloom Documentation Tools on Darwin
...
```sh
git clone https://github.com/moonman81/heirloom-doctools-darwin
cd heirloom-doctools-darwin
```
...
Gunnar Ritter maintained. Nothing in `heirloom-doctools-darwin` alters
...
writes `.install-manifest-doctools.txt` at the repo root. That
...
Reads `.install-manifest-doctools.txt`, removes every listed file
